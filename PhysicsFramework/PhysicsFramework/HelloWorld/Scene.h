...
inline void to_json(nlohmann::json& j, const Scene& scene)
{
	...
	for (auto entity : view)
	{
		...
		if (identity & EntityIdentifier::HoriScrollCameraBit())
		{
			...
			j[to_string(counter)]["HoriScrollCam"] = scene.GetScene()->get<HorizontalScroll>(entity);
		}
		if (identity & EntityIdentifier::VerticalScrollCamBit())
		{
			//Stores the horizontal Scrolling Camera
			j[to_string(counter)]["VerticalScrollCam"] = scene.GetScene()->get<VerticalScroll>(entity);
		}
...
inline void from_json(const nlohmann::json& j, Scene& scene)
{
	...
	for (unsigned i = 0; i < numEntities; i++)
	{
		...
		if (identity & EntityIdentifier::HoriScrollCameraBit()) 
		{
			//Adds Horizontal scrolling camera to the entity
			reg.assign<HorizontalScroll>(entity);
			//Sets the Horizontal scrolling camera to our saved version
		...
		if (identity & EntityIdentifier::VerticalScrollCamBit())
		{
			...
			//Sets the Horizontal scrolling camera to our saved version
			reg.get<VerticalScroll>(entity) = j["Scene"][std::to_string(i)]["VerticalScrollCam"];
	...
	if (scrollHori)
	{
		...
		//Makes the camnera focus on the MainPlayer

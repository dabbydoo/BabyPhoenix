...
Selectable EffectManager::m_selectable = Selectable("Assignment Editor");
	...
	if (m_selectable.GetSelected())
	{
		if (ImGui::TreeNode("Sepia Effect"))
		{
			...
			else
			{
				...
				if (ImGui::Button("Add Efffect", ImVec2(100.f, 20.f)))
				{
		...
		auto& player = ECS::GetComponent<Transform>(EntityIdentifier::MainPlayer());
		...
		if (ImGui::TreeNode("Horizontal scroller"))
		{
			...
			{
				ECS::GetComponent<Transform>(EntityIdentifier::MainPlayer()).SetPosition(vec3(x1, y1, z1));
			}

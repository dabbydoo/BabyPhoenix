...
void XInputController::GetSticks(Stick sticks[2])
{
	...
	if (sqrt(x * x + y * y) < m_deadZoneStick)
		...
		sticks[1].x = float(m_info.Gamepad.sThumbRX / 32768);
	else
		sticks[1].x = float(m_info.Gamepad.sThumbRX / 32767);
	...
	if (sqrt(x * x + y * y) < m_deadZoneStick)
		...
		sticks[1].y = float(m_info.Gamepad.sThumbRY / 32768);
	else
		sticks[1].y = float(m_info.Gamepad.sThumbRY / 32767);
}

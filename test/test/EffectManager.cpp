...
Selectable EffectManager::m_selective=Selectable("Test Editor");
...
{
	bool temp = false;
	if (ImGui::BeginTabItem(m_selective.GetName().c_str(),m_selective.GetSelected())) {
		temp = true;
		ImGui::EndTabItem();
	}
	if (temp) {
...
{
#pragma region Sepia
	if (m_selective.GetSelected()) {
		if (ImGui::TreeNode("Sepia Effect")) 
		{
			if (m_sepia!= -1)
			{
				SepiaEffect* temp = (SepiaEffect*)EffectManager::GetEffect(m_sepia);
				float intensify = temp->GetIntensity();
			
				ImGui::Text("Currently Attached");
			
				if (ImGui::DragFloat("Intensify",&intensify,0.01f,0.f,1.f))
				{
					temp->SetIntensity(intensify);
				}
...
auto& player = ECS::GetComponent<Transform>(6);
float x1= player.GetPosition().x;
float y1= player.GetPosition().y;
float z1 = player.GetPosition().z;

if (ImGui::TreeNode("Horizontal scroller"))
{
	if (ImGui::DragFloat("Horizontal Scroll", &x1,1.f, -150.f, 150.f))
	{
		ECS::GetComponent<Transform>(6).SetPosition(vec3(x1, y1, z1));
	}

	ImGui::TreePop();
}
if (ImGui::TreeNode("Veritcal scroller"))
{
	if (ImGui::DragFloat("Vertical Scroll", &y1, 1.f, -150.f, 150.f))
	{
		ECS::GetComponent<Transform>(6).SetPosition(vec3(x1, y1, z1));
	}
...
auto& playercam = ECS::GetComponent<Camera>(0);
float z2 = player.GetRotationAngleZ();

if (ImGui::TreeNode("Rotate"))
{
	if (ImGui::SliderAngle("z-axis change", &z2, -360, 360))
	{
		playercam.SetRotationAngleZ(z2);
	}

	ImGui::TreePop();
}
auto& animController = ECS::GetComponent<AnimationController>(4);



if (ImGui::Button("Animation 1", ImVec2(150.f, 30.f))) {
	
	animController.SetActiveAnim(1);
}

if (ImGui::Button("Animation 0", ImVec2(150.f, 30.f))) {
